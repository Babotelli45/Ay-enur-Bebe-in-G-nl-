...
export default function DateNav({
  ...
  return (
    <div className="absolute top-3 right-3 sm:top-5 sm:right-8 z-10 flex flex-col items-end gap-1">
      <div className="flex items-center gap-2 bg-white/70 rounded-full px-2 py-1 shadow-sm">
        <button
          onClick={onPrev}
          disabled={!canPrev}
          className="w-7 h-7 rounded-full hover:bg-white disabled:opacity-30 font-label text-ink"
      ...
      <input
        ...
        className="text-[11px] font-label rounded-full bg-white/60 px-2 py-0.5 text-ink outline-none"
      />
    </div>
  );
}
